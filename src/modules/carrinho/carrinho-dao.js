...
class CarrinhoDao {
  ...
  getCarrinho(res, id_cliente){
    executeSQL('SELECT p.* FROM ecommerce.produto p INNER JOIN ecommerce.carrinho car ON car.id_produto = p.idproduto INNER JOIN ecommerce.cliente c ON car.id_cliente = c.id_usuario WHERE c.id_usuario = '+id_cliente+';').then(carrinho => {
      res.json(carrinho);
    })
    .catch(err => {
      console.log(err);
  ...
  confirmaProdutoNoCarrinho(res, id_cliente, id_produto){
    executeSQL('SELECT id_produto FROM ecommerce.carrinho WHERE id_cliente = '+id_cliente+' AND id_produto = '+id_produto+';').then(confirmacao => {
      if (confirmacao[0]) {
        res.json(true);
      }else{
        res.json(false);
  ...
  addAoCarrinho(res, id_cliente, id_produto){
    executeSQL('INSERT INTO ecommerce.carrinho(id_cliente, id_produto, data_inclusao) VALUES('+id_cliente+', '+id_produto+', "'+dataNow()+'");').then(addProdutoCarrinho => {
  ...
  remDoCarrinho(res, id_cliente, id_produto){
    executeSQL('DELETE FROM ecommerce.carrinho WHERE id_cliente = '+id_cliente+' AND id_produto = '+id_produto+';').then(remProdutoCarrinho => {
